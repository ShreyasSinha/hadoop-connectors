...
public class FsBenchmark extends Configured implements Tool {
  ...
  private void benchmarkVectoredRead(FileSystem fs, Path testFile) throws IOException {
    try (FSDataInputStream input = fs.open(testFile)) {
      ...
      List<? extends FileRange> ranges = getTestRanges();
      System.out.println("Calling Read Vectored");
      input.readVectored(ranges, allocator);
      System.out.println("Read Vectored Call Completed");
      ranges.forEach(
          range -> {
            try {
              System.out.println("Printing Read Vectored");
              ...
              String decodedString = charset.decode(data).toString();
  ...
  private List<? extends FileRange> getTestRanges() {
    List<FileRange> ranges = new ArrayList<>();
    ranges.add(new CustomFileRange(300, 400));
    ranges.add(new CustomFileRange(750, 200));
    ranges.add(new CustomFileRange(500, 100));
    ranges.add(new CustomFileRange(500, 200));
    ranges.add(new CustomFileRange(700, 150));
    ranges.add(new CustomFileRange(750, 150));
    ranges.add(new CustomFileRange(450, 150));
    ranges.add(new CustomFileRange(20, 550));
    ranges.add(new CustomFileRange(700, 850));
    ranges.add(new CustomFileRange(850, 150));
    return ranges;
  }
